...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function openCommunityBlockModal(button) {
    const { handle } = getCommunityUserMeta(button); closeMoreDropdown(); closeCommunityModal();
    ...
    modal.addEventListener('click', (e) => { if (e.target.classList.contains('notification-dialog__backdrop') || e.target.closest('.notification-dialog__close')) { e.preventDefault(); closeCommunityModal(); return; } if (e.target.closest('.notification-dialog__confirm-block')) { e.preventDefault(); showCommunityToast(`${handle} 님을 차단했습니다`); closeCommunityModal(); } });
  ...
  function openCommunityReportModal() {
    ...
    modal.addEventListener('click', (e) => { if (e.target.classList.contains('notification-dialog__backdrop') || e.target.closest('.notification-dialog__close')) { e.preventDefault(); closeCommunityModal(); return; } if (e.target.closest('.notification-report__item')) { e.preventDefault(); showCommunityToast('신고가 접수되었습니다'); closeCommunityModal(); } });
    document.body.appendChild(modal); document.body.classList.add('modal-open'); activeCommunityModal = modal;
  }
  function handleMoreDropdownAction(button, actionClass) {
    const { handle } = getCommunityUserMeta(button);
    if (actionClass === 'menu-item--follow-toggle') { const isF = communityFollowState.get(handle) ?? false; communityFollowState.set(handle, !isF); closeMoreDropdown(); showCommunityToast(isF ? `${handle} 님 팔로우를 취소했습니다` : `${handle} 님을 팔로우했습니다`); return; }
  ...
  function getMoreDropdownItems(button) {
    const { handle } = getCommunityUserMeta(button); const isF = communityFollowState.get(handle) ?? false;
    return [
